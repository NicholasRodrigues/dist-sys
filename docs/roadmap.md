...
| 7. Quando o antifraude cai | 3 | T5 | Parar o `risk-api` ao vivo. `fail_open` vende, `fail_closed` bloqueia, chave virando em tempo de execução. **Não há resposta técnica — é decisão de produto**, e por isso mora numa flag. O breaker segurando o checkout em ~194 ms |
